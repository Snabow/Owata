# WP-004 Entry Audit — Phase 4 Router
...
**Scope:** Audit only — no Router implementation in this request.
...
`WP-004-S1 — Provider Registry + Capability Profile + deterministic eligibility filter`
...
| --- | --- | --- | --- | --- | --- | --- |
| P4-01 | Provider registry | TRUE_PHASE4_GAP | Hardcoded Champion bindings only (`cursor-cli`, `codex-cli`); Charter §7 provisional Champions | Durable/provider-neutral registry of runtimes/bindings | **YES** | Define registry records + load/list API |
...
| P4-04 | Quota | TRUE_PHASE4_GAP | Canary budget only (`src/canary/b2-full-invocation-budget.ts`) — non-product | Durable quota representation before live APIs | No | Deferred: config/observed budget object later |
...
| P4-06 | Automatic escalation | DEFERRED_WITHIN_PHASE4 | Charter §5.7; DEC-003-002 future escalate | Policy for stronger model/provider under constraints | No | After registry+eligibility |
...
| P4-08 | Role capability preflight | SUBSTRATE_EXISTS | Dispatcher preflight + `CAPABILITY_BLOCK` | None for Phase 4 product | No | Reuse; do not rebuild as “Capability Profile” |
| P4-09 | Champion/Challenger R&D process | NON_GOAL (first slice) | Charter §7 process | — | No | Not a Router module |
...
| Probe availability | Binding `probe()` → gateway preflight | Feed eligibility; not multi-provider yet |
| Failure classes | `AGENT_UNAVAILABLE`, `CREDENTIAL_UNAVAILABLE`, `CAPABILITY_BLOCK` | Map NO_ELIGIBLE / auth / agent failures |
| Model-policy (canary/Builder) | `src/execution/model-policy.ts` | **Local experiment/canary policy** — must not become universal architecture |
...
| **Human** | Goals, constraints, approval, credential recovery — **not** routine provider/model Clipboard |
...
| Eligibility decisions (audit) | Prefer durable event/log | Reconstructable “why this binding” |
| Probe observations | Ephemeral cache OK | May refresh; do not treat session as authority |
| Quota/cost observations | Deferred; when added → durable | Never scrape plan limits into source |
...
- New Agent/provider evaluation / Champion–Challenger experiments as product Router  
- Universal “never use Claude” (canary model-policy ≠ architecture)  
...
**Name:** `WP-004-S1 — Provider Registry + Capability Profile + deterministic eligibility filter`
...
Empty eligibility maps to existing fail-closed outcomes (`CAPABILITY_BLOCK` / `AGENT_UNAVAILABLE` / `CREDENTIAL_UNAVAILABLE` as appropriate)—not silent fake fallback.
...
## Expected source touch points (future S1 — not this audit)
...
| Quota minimum? | Deferred: durable observed/configured budget object — no scraping plan limits |
| Cost minimum? | Deferred: measured / configured estimate / unknown — never bake volatile prices into Core |
...
| Current bindings? | Cursor/Codex hardcoded in bindings + canaries — abstract behind registry in S1; leave canary model remaps as experiment policy |
| Model policy vs Router? | Local Builder/canary non-Claude policy ≠ universal prohibition; Router must remain provider-agnostic |
| Durable vs ephemeral? | Registry/profiles/policy durable; probe cache ephemeral |
| Failure is state? | Unavailable / no-eligible / auth → recoverable Control Core classes, not orphaned work |
...
| Classification | Count (matrix rows P4-01…P4-09) |
| --- | --- |
