...
| Availability (S2) | EXISTING | Runtime readiness 竕 quota |
...
## 3. Dispatch integration 窶・ABSENT (critical unlock)

### Where role 竊・concrete runtime happens today
...
| Gateways | `src/program-control/gateway.ts`, `src/execution/gateway.ts`, `src/reviewer/gateway.ts` | Each takes **one** binding; `identity.adapter_id` is gateway-class (`gateway-builder` etc.), **竕** registry `binding_id` |
...
- `filterEligibleBindings` 窶・`src/router/eligibility.ts`
- `overlayAvailability` / `normalizeAvailability` / `observeBindingAvailability` 窶・`src/router/availability.ts`
- `loadProviderRegistry` / parse helpers 窶・`src/router/registry.ts`

### Minimum future seam (do not implement in this audit)

```text
loadProviderRegistry 竊・filterEligibleBindings 竊・overlayAvailability 竊・selectFirstAvailableBinding
```
...
## 4. Automatic escalation 窶・ABSENT (deferred)
...
| Prior failure evidence | Partial (`FailureClass`, recovery); no 窶徼oo weak model窶・|
| Strength ranking beyond `priority` | No |

| Kind | Who | Location today |
| --- | --- | --- |
| Semantic PC decision | Program Control | `PcDecisionBody` 竊・`assertStrictProgramControlDecision` 竊・`assertPcDecisionAuthority` 竊・`Dispatcher.applyDecision` |
| Router binding choice | Router (future) | Absent 窶・canary hardcodes Champions |
| Recovery after runtime failure | Control Core 竊・PC | `enterRecovery`, retry budget 竊・`RECOVERY_REQUIRED` |
| Stronger provider / model policy | Local only | `src/execution/model-policy.ts` (A-002: remain outside generic Router) |

Escalation 竕 REWORK. Router must never decide BUILD/REWORK/ACCEPT/HUMAN_GATE.
...
| Fencing | YES | `fence_token`; stale 竊・`RESULT_STALE` / `STALE_FENCE` |
...
| Exact binding identity on dispatch | **NO** | `dispatches` has no `binding_id`; `adapter_id` 竕 registry `binding_id` |
| Retry lineage | YES (semantic) | `recovery_lineage_id`, `retry_of_request_id` 窶・PC recovery, not binding failover |
...
2. either honest coarse states (`AVAILABLE` / `EXHAUSTED` / `UNKNOWN`) with adapter-owned probes, or an explicit provider-adapter boundary 窶・not false numeric normalization.
...
| SQLite schema change in next slice | **NO** 窶・not required for acceptance |

---

## Dependency order (evidence-driven)

1. **Deterministic binding selection** over S1竏ｩS2 (unlocks consumption of what already exists)
...
### WP-004-S3 窶・Deterministic binding selection from S1竏ｩS2 (API-only; no active cutover)

**In scope:** pure `selectBinding` (or equivalent) composing eligibility + availability 竊・`SELECTED` | `NO_ELIGIBLE_BINDING` | `NO_AVAILABLE_BINDING`; unit tests; Decision for A-004/A-005 defaults.
...
### A-004 窶・Single-binding selection rule after S2 overlay
...
### A-005 窶・Cutover timing vs API-only S3
...
### A-006 窶・Map unavailable outcomes 竊・Control `FailureClass`
...
- **Evidence:** DEC-004-002: S2 does not map to FailureClass; gateways today collapse probe fail into missing `command_execution` 竊・`CAPABILITY_BLOCK`.
...
### A-007 窶・Persist `binding_id` on `dispatches` (or durable routing evidence)

- **Question:** Before failover, must claim/accept records include registry `binding_id`?
- **Evidence:** `dispatches` has no binding column; `adapter_id` 竕 `binding_id`.
- **Recommended default:** YES as prerequisite for failover; **not** required for API-only S3.

### A-008 窶・`adapter_id` vs `binding_id` identity model
...
| QUOTA_STATUS | ABSENT |
| COST_STATUS | ABSENT |
| DISPATCH_INTEGRATION_STATUS | Role竊辰hampion hardcoded in canary/gateways; Dispatcher has no Router import; S1+S2 APIs unwired |
